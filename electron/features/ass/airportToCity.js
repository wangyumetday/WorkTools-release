// ============================================================
// 机场三字码 → 城市三字码 转换（ASS 模块独立副本）
//
// 设计要点（与 PCP 模块逐行一致，但作为 ass 独立模块存在，
//   绝不 import / require / 调用任何 pcp 路径下的文件或对象 — ADR-4 红线）：
//   1. 只存"机场码 ≠ 城市码"的 67 条映射，不存相同的（省内存）
//   2. 查不到时回退为机场码本身（覆盖所有非 MAC 机场，多数机场码 = 城市码）
//   3. 模块级常量，进程加载即驻留，O(1) 查询
//
// 数据来源：IATA Resolution 011c（41 个多机场城市 MAC）+ 历史城市码补齐
// ============================================================
...
const MAC_MAPPING = {
  ...
  // TH 曼谷
  DMK: 'BKK',
  // TR 伊兹密尔（单机场城市，但城市码沿用历史码 IZM ≠ 机场码 ADB）
  ADB: 'IZM',
  // TR 安卡拉
  ESB: 'ANK',
  // TR 伊斯坦布尔
 ...
 * @returns {string}  城市三字码（如 'SHA'）；查不到则返回原始机场码（多数机场码=城市码）
 */
