<!-- ============================================================
     PCP TaskListVirtual.vue - 任务列表(虚拟列表实现,保留方案)
     设计:
       - n-virtual-list 只挂载可视区 item,支持超大任务量
       - :item-size="42" 固定行高(详情展开后行高变化,滚动测算会有偏差)
       - 详情内嵌行下,滚出视窗会随 item 卸载(虚拟化固有行为)
     与 TaskList.vue 共享 useTaskList 逻辑 + taskList.css 样式
     说明: 几百~千任务量下推荐用 TaskList.vue(真实列表),
           本文件保留以备任务量超大时切换回去。
     ============================================================ -->
<template>
  <div class="tab-list">
                ...
                <template v-else>
                  ...
                  <template v-if="getJxgjTotalFlights(item) === 1">
                    <div class="tld-meta">
                      ...
                      <span class="tld-chip__sep"></span>
                      <span class="tld-chip">
                        <span class="tld-chip__label">成人总票价</span>
                      ...
                      <div v-for="d in getJxgjDateEntries(item)"
                        ...
                        <div v-if="expandedDates.has(d.date)">
                          <table class="tld-flight-table">
                            <thead>
                              <tr>
                                <th>航班</th><th>舱位</th><th>成人总票价</th><th>底价</th>
                              </tr>
                            </thead>
                            <tbody>
...
<script setup>
import { ref } from 'vue'
import { NVirtualList, NTag } from 'naive-ui'
import { useTaskList } from '../composables/useTaskList.js'

// 预留:如以后要恢复"滚动到运行中任务",用此 ref 调 virtualListInst.scrollTo({ index })
...
<!-- 共享样式(与 TaskList.vue 同源):行/详情/颜色 -->
<style scoped src="./taskList.css"></style>

<!-- 虚拟列表专属:让 n-virtual-list 充满 .tab-list 容器 -->
<style scoped>
.tab-list__scrollbar {
  width: 100%;
}
